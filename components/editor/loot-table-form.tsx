...
import { ChangeEvent, FocusEvent, ReactNode, useCallback, useEffect, useMemo, useRef, useState, useTransition } from 'react';
import Link from 'next/link';
import { ChevronDown, ChevronRight, ChevronsUpDown, Copy, Download, FlaskConical, Plus, Rocket, Save, Share2, Trash2, Upload, Wand2 } from 'lucide-react';
...
/** Item shape the editor consumes (mapped from the game manifest at the page). */
export interface EditorItem {
  id: string;
  name: string | null;
}

/** No-op stand-in for the removed realtime-collab presence layer (deferred). */
...
export function LootTableEditor({
  ...
  // Optimistic-lock counter: the revision the server last acknowledged.
  const revisionRef = useRef(initialRevision);
  // Realtime collaboration is deferred — these are inert stand-ins so the rest
  // of the editor (presence-wrapped fields, broadcast calls) compiles unchanged.
  const others: never[] = [];
  const trackFocus = (_fieldId?: string) => {};
  const untrackFocus = () => {};
  const broadcastPatch = (_path: string, _value: unknown) => {};
  void setDefinitionState;
    ...
    startPublishing(async () => {
      ...
      if (autosave.dirty) {
        try {
          ...
          return; // save error already surfaced
        }
      }
      const draft = {
        ...definition,
        guaranteed: definition.guaranteed.map((entry) => ({ ...entry, weight: 1 })),
      };
      const result = await publishAction({ id: tableId, type: 'loot_table', draft });
      if (!result.ok) {
        const detail = result.issues ? `: ${result.issues.join('; ')}` : '';
        setError(`${result.error}${detail}`);
        return;
      }
      addNotification(`Published v${result.version} — live in game on reload`, 'success');
    });
  };
  ...
  const handleDelete = () => {
    ...
    startDeleting(async () => {
      ...
      // Server action redirects to the index on success.
      await deleteContentAction('loot-tables', tableId);
    });
  };
      ...
      <div className="sticky top-0 z-10 flex flex-wrap items-center justify-between gap-4 rounded-lg border bg-card/95 px-6 py-4 backdrop-blur-sm">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button
            ...
            disabled={isPublishing || autosave.status === 'saving'}
          >
            <Rocket className="h-4 w-4" />
            {isPublishing ? 'Publishing...' : 'Publish'}
          </Button>
          <VersionHistory contentId={tableId} />
